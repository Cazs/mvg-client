...
public class Trip extends MVGObject
{
    ...
    public Quote getQuote()
    {
        ...
        {
            Quote quote = QuoteManager.getInstance().getQuotes().get(quote_id);
            if(quote!=null)
            {
                //return latest revision
                Quote[] revisions = quote.getSortedSiblings("revision");
                return revisions[revisions.length - 1];
            } else IO.logAndAlert(getClass().getName(), IO.TAG_WARN, "Quote["+quote_id+"] was not found in the database.");
        } else IO.logAndAlert(getClass().getName(), IO.TAG_WARN, "No quotes were found in the database.");
        return null;
    }
